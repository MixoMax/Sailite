document.addEventListener('DOMContentLoaded', () => {
    ...
    const BASE_HAND_SCORES = { /* ... (no changes) ... */
    ...
    const TAROT_POOL = [ /* ... (no changes) ... */
        { id: "t_strength", name: "Strength", description: "Add +30 Chips to 2 selected cards", effect: { type: 'enhance_card', enhancement: 'bonus_chips', value: 30, targets: 2, requiresTarget: true } }, { id: "t_death", name: "Death", description: "Convert 1 selected card to match the suit of another selected card", effect: { type: 'convert_suit', targets: 2, requiresTarget: true } }, { id: "t_fool", name: "The Fool", description: "Create a copy of the last played Consumable card (if any)", effect: { type: 'create_last_consumable', targets: 0, requiresTarget: false } }, { id: "t_emperor", name: "The Emperor", description: "Gain $5", effect: { type: 'gain_money', value: 5, targets: 0, requiresTarget: false } }, { id: "t_chariot", name: "The Chariot", description: "Apply Foil edition to 1 selected card", effect: { type: 'apply_edition', edition: 'foil', targets: 1, requiresTarget: true } }, { id: "t_star", name: "The Star", description: "Apply Holographic edition to 1 selected card", effect: { type: 'apply_edition', edition: 'holographic', targets: 1, requiresTarget: true } }, { id: "t_sun", name: "The Sun", description: "Apply Polychrome edition to 1 selected card", effect: { type: 'apply_edition', edition: 'polychrome', targets: 1, requiresTarget: true } }, { id: "t_judgement", name: "Judgement", description: "Apply Red seal to 1 selected card", effect: { type: 'apply_seal', seal: 'red', targets: 1, requiresTarget: true } }, { id: "t_temperance", name: "Temperance", description: "Apply Blue seal to 1 selected card", effect: { type: 'apply_seal', seal: 'blue', targets: 1, requiresTarget: true } }, { id: "t_wheel", name: "Wheel of Fortune", description: "Apply Gold seal to 1 selected card", effect: { type: 'apply_seal', seal: 'gold', targets: 1, requiresTarget: true } }, { id: "t_hermit", name: "The Hermit", description: "Apply Purple seal to 1 selected card", effect: { type: 'apply_seal', seal: 'purple', targets: 1, requiresTarget: true } },
    ];

    // --- Voucher Definitions ---
    const VOUCHER_POOL = [ /* ... (no changes) ... */
    ...
    function createDeck() { /* ... (no changes) ... */
        deck = []; for (const suit of SUITS) { for (const rank of RANKS) { deck.push({ suit, rank, id: `${rank}${suit[0].toUpperCase()}`, enhancement: null, edition: null, seal: null }); } }
    }
    function shuffleDeck() { /* ... (no changes) ... */
        for (let i = deck.length - 1; i > 0; i--) { const j = Math.floor(Math.random() * (i + 1)); [deck[i], deck[j]] = [deck[j], deck[i]]; }
    }
    function drawCards(numCards) { /* ... (no changes) ... */
        const drawn = []; for (let i = 0; i < numCards && deck.length > 0; i++) { drawn.push(deck.pop()); } if (drawn.length < numCards && discardPile.length > 0) { console.log("Deck empty, reshuffling discard pile..."); deck = [...discardPile]; discardPile = []; shuffleDeck(); for (let i = drawn.length; i < numCards && deck.length > 0; i++) { drawn.push(deck.pop()); } } return drawn;
    }
    ...
    function toggleCardSelection(cardElement, cardData) { /* ... (no changes) ... */
    ...
    function updatePotentialHandDisplay() { /* ... (no changes) ... */
    ...
    function evaluatePokerHand(cards) { /* ... (no changes) ... */
    ...
    // --- Scoring ---
    function getCardChipValue(card) { /* ... (no changes) ... */
    ...
    function isEvenRank(rank) { /* ... (no changes) ... */
    ...
    // --- Planet Card Logic ---
    function usePlanetCard(planetCard) { /* ... (no changes) ... */
    ...
    function useConsumable(consumable, index) { /* ... (no changes) ... */
    ...
    function applyTarotEffectToTarget(targetCardData, nonTargetEffect = null) { /* ... (no changes) ... */
    ...
    function cancelTarotTargeting(renderConsumablesToo = true) { /* ... (no changes) ... */
    ...
    // --- Voucher Logic ---
    function applyVoucherEffects() { /* ... (no changes) ... */
    ...
    function getAvailableVoucher() { /* ... (no changes) ... */
    ...
    function hasVoucher(voucherId) { /* ... (no changes) ... */
        return purchasedVouchers.includes(voucherId);
    }

    // --- Card Effects Logic ---
    function applyRandomCardEffects(card) { /* ... (no changes) ... */
    ...
    function renderJokers() { /* ... (no changes) ... */
    ...
    function renderConsumables() { /* ... (no changes) ... */
    ...
    function updateHandLevelDisplay() { /* ... (no changes) ... */
    ...
    function updatePurchasedVouchersDisplay() { /* ... (no changes) ... */
    ...
    function populateShop() { /* ... (no changes) ... */
    ...
    function buyItem(item, type, shopIndex, slotElement) { /* ... (no changes) ... */
    ...
    function sellJoker(index) { /* ... (no changes) ... */
    ...
    function rerollShop() { /* ... (no changes) ... */
    ...
    sortRankBtn.addEventListener('click', () => { /* ... (no changes) ... */
        if (isAnimating) return; hand.sort((a, b) => RANK_ORDER[a.rank] - RANK_ORDER[b.rank]); renderHand();
    });
    sortSuitBtn.addEventListener('click', () => { /* ... (no changes) ... */
